 ...
 *
 * @param {Object} popupDialog - Information about the popup.
 * @param {Object} popupDialog.popup - The popup object returned from
 * window.open.
 * @param {Object} popupDialog.transport - The transport instance used for
 * communication with the popup window.
 * @returns {{
 *     type: SET_DEVICE_SELECTION_POPUP,
 *     popupDialog: Object
 * }}
 */
function _setDeviceSelectionPopup(popupDialog) {
    return {
        type: SET_DEVICE_SELECTION_POPUP,
        popupDialog
    };
...
function _openDeviceSelectionDialogInPopup() {
    return (dispatch, getState) => {
        const { popupDialog } = getState()['features/device-selection'];
        ...
        const popup
            = window.open(url, 'name', 'toolbar=no,scrollbars=no,resizable=no');
        ...
        transport.on('event', event => {
            if (event.type === 'devices-dialog' && event.name === 'close') {
                popup.close();
                transport.dispose();
                dispatch(_setDeviceSelectionPopup());
